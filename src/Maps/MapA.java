    ...
    public MapA(Game g, int cellHeightPX, int cellWidthPX){
    	...
    	try {  		
			...
			while ( (actualLine = br.readLine()) != null) {
				...
				System.out.println("Procesando " + actualLine);
				
				for(int col = 0; col<20; col++){
					

					if( actualLine.charAt(col) == 'A') {
					...
					System.out.println("Fila " + actualRow + " Columna " + col);	
			...
			while ( ( num = (char) br.read() ) != -1) {
				...
				for(int row = 0; row<20; row++){
					for(int col = 0; col<20; col++){
						...
						if(num == 'A') {
							cells[row][col] = Wall.cloneInPosition(row, col);
						}
						if(num == 'B') {
							cells[row][col] = RoadPacdot.cloneInPosition(row, col);
						}
